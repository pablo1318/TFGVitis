...
// ==============================================================
`timescale 1ns/1ps
module kvadd_tutorial_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_B_DATA_1        = 6'h28,
    ADDR_B_CTRL          = 6'h2c,
    ADDR_RES_DATA_0      = 6'h30,
    ADDR_RES_DATA_1      = 6'h34,
    ADDR_RES_CTRL        = 6'h38,
    WRIDLE               = 2'd0,
    WRDATA               = 2'd1,
    WRRESP               = 2'd2,
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_RES_DATA_0)
            int_res[31:0] <= (WDATA[31:0] & wmask) | (int_res[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_RES_DATA_1)
            int_res[63:32] <= (WDATA[31:0] & wmask) | (int_res[63:32] & ~wmask);
    end
end
