...
entity kvadd_tutorial_control_s_axi is
generic (
...
end entity kvadd_tutorial_control_s_axi;
...
architecture behave of kvadd_tutorial_control_s_axi is
    ...
    constant ADDR_RES_DATA_0      : INTEGER := 16#30#;
    constant ADDR_RES_DATA_1      : INTEGER := 16#34#;
    constant ADDR_RES_CTRL        : INTEGER := 16#38#;
    ...
    signal int_res             : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_RES_DATA_1 =>
                        rdata_data <= RESIZE(int_res(63 downto 32), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_RES_DATA_0) then
                    int_res(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_res(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_RES_DATA_1) then
                    int_res(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_res(63 downto 32));
                end if;
            end if;
        end if;
    end process;
