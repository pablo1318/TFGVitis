...
int main(int argc, char** argv)
{
    ...
    //
    kernel = clCreateKernel(program, "kernel_prueba", &err);
    ...
    err = 0;
    cl_int d_in1 = 0;
    err |= clSetKernelArg(kernel, 0, sizeof(cl_int), &d_in1); // Not used in example RTL logic.
    cl_int d_in2 = 0;
    err |= clSetKernelArg(kernel, 1, sizeof(cl_int), &d_in2); // Not used in example RTL logic.
    ...
    for (cl_uint i = 0; i < number_of_words; i++) {
        if ((h_data[i] + 1) != h_axi00_ptr0_output[i]) {
            printf("ERROR in kernel_prueba::m00_axi - array index %d (host addr 0x%03x) - input=%d (0x%x), output=%d (0x%x)\n", i, i*4, h_data[i], h_data[i], h_axi00_ptr0_output[i], h_axi00_ptr0_output[i]);
            check_status = 1;
        }
